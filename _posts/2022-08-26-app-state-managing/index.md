...
위 두 지표를 개선하기 위해 현재 팀 내에 존재하는 문제 상황들을 분석하고, 적절한 솔루션을 찾아 해결하는 과정을 아래에 적어보려 합니다.

### 팀 내에 존재하는 문제상황

#### 0. 프로덕트 특성상 깊고 복잡한 상태들이 존재하며, 이런 상태의 변경은 복잡한 도메인 구조를 이해하지 않는다면 쉽지 않다.
타다 드라이버 앱은, 일반적인 앱들에 비해 깊고 복잡한 State Tree 를 가지고 있습니다. 기본적인 앱 로그인 / 로그아웃 상태에, 현재 택시를 운행중인지, 운행 중이라면 고객을 태운 상황인지, 운행과 별도로 존재하는 다음에 수행할 예약 운행이 있는지 등등... 이와 같은 깊고 복잡한 (Nested & Deep) 상태를 잘 관리할 수 있도록 RIBs 라는 아키텍쳐를 선택한 것 또한 위의 상황을 잘 보여주고 있습니다.

> 현재 타다 드라이버 앱 내의 State Tree 를 유추할 수 있는 RIB Tree

복잡한 상태를 가진 만큼 그 상태 변경 로직은 다양한 상황 및 도메인 로직을 고려해야 합니다. 서버와 클라이언트 간의 상태를 싱크맞출 때도, 사용자의 인지 여부에 따라 특정 상태를 업데이트 하지 않는다, X 상황에서는 Y 의 업데이트를 지연한다, 와 같은 지식이 충분하지 않은 상태에서 상태 변경 코드를 작성하면 버그를 만들어 낼 가능성이 높습니다,.

```kotlin
fun updateStateAfterDroppedOffRide(/* ...  */) {
  // ...
...
```
> 상태 업데이트에서 발생하는 상황적 맥락은 위 예시 코드 등을 통해서 간접 체험할 수 있습니다.

#### 0. 상태의 변경이 발생하는 아키텍쳐 계층이 너무 높다. (서버의 상태랑 클라이언트 상태의 차이가 존재할 수 있다.)

타다 드라이버 앱은, 일반적인 앱들에 비해 깊고 복잡한 State Tree 를 가지고 있습니다. 기본적인 앱 로그인 / 로그아웃 상태에, 현재 택시를 운행중인지, 운행 중이라면 고객을 태운 상황인지, 운행과 별도로 존재하는 다음에 수행할 예약 운행이 있는지 등등...
이와 같은 깊고 복잡한 (Nested & Deep) 상태를 잘 관리할 수 있도록 RIBs 라는 아키텍쳐를 선택한 것 또한 위의 상황을 잘 보여주고 있습니다.

![RIB Architecture](./thumbnail-color.png)

복잡한 상태를 가진 만큼 상태 변경 로직은 신중해야 하며, 도메인 지식이 충분하지 않은 상태에서는 크리티컬한 버그를 만들어 낼 가능성이 높습니다.
그러나 타다 드라이버 코드베이스는 이런 상태 변경 로직이 특정 추상화 계층에 몰려있지 않고, 여기저기에 산재되어 있다 보니 실수로 잘못된 변경을 만들어 낼 가능성이 높았고, 이런 버그에 대한 안정성을 보장하는 유닛 테스트들도 작성하기 어려운 상황이었습니다.
