...
## 도메인 정리를 시작하게 된 이유
타다 서버팀이 최근 마주한 가장 큰 문제 중 하나는 코드 상 도메인 간의 강결합에 따른 복잡성이라고 생각했습니다. 
이를 해결하기 위해 에릭 에반스의 도메인 주도 설계를 스터디하였습니다. 
그리고 기획자/도메인 전문가와 개발자 사이의 다른 이해도와 그에 따른 단방향적 커뮤니케이션과 구현이 코드의 복잡성을 가중시킬 수 있다는 것을 깨달았으며

서비스가 고도화됨에 따라 확장성을 늘리고 더 효율적인 의사소통을 해나가기 위해서는 코드-모델-설계를 유기적으로 관리하는 것이 필요하다고 생각했으며,
정산 도메인을 시작으로 도메인 정리를 해 나아가고 있습니다. 

## 왜 정산일까
정산 도메인은 크게 두 가지 특징이 있었습니다.

첫째, `코드의 복잡성`이 매우 높아 `기능을 추가하는 데에 큰 어려움`이 있었습니다. 정산 자동화 기능을 인터널 팀에서 개발을 진행하려고 했으나, 
...
위와 같은 이유로 정산 도메인을 첫 도메인 정리 대상으로 정하였고, 모델 주도 설계를 적용해보기로 했습니다. 
...
기존 구현에 대한 모델을 그리고 나니 문제점과 핵심이 한 눈에 보이기 시작했습니다. 가장 큰 문제는 객체의 역할이 명확하지 않다는 점이었습니다.
...
구현의 복잡도가 매우 올라갈 것으로 예상되었습니다. 세 기준으로 나눈 금액인 `SettlementDivision`을 계산하고,
...
![previous-code-1]( ./previous-code-1.png)
![previous-code-2]( ./previous-code-2.png)
![previous-code-3]( ./previous-code-3.png)
![previous-code-4]( ./previous-code-4.png)
