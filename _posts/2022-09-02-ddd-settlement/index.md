...
타다 서버팀 역시 이러한 문제를 겪고 있었고, 
도메인 사이의 강결합과 더불어 도메인 내부의 높은 복잡도를 해결하기 위해
...
도메인 주도 설계를 통해 이를 해결하며, 모델을 중심으로 기획자/설계자와 개발자가 긴밀하게 협업하는 것이 핵심입니다. 
...
`첫째`, 정산은 결제된 금액을 드라이버와 VCNC에 분배하는 과정입니다. 따라서 정산은 서비스 흐름의 **마지막**에 존재하며, 비교적 다른 도메인과의 **결합도가 낮습니다**. <br/>
`둘째`, 도메인 지식과 코드의 **복잡성**이 매우 높아 **기능을 추가하는 데에 큰 어려움**이 있었습니다.
...
## 기존 정산 도메인 파악하기
### 도메인 지식 및 구현 파악하기
정산 도메인에 대한 지식과 코드가 주기적으로 관리되지 않고 있었기 때문에 <br/>
...
### 기존 모델 도출
기존에는 모델이 정의되어 있지 않았었습니다. 핵심과 흐름을 한 눈에 볼 필요가 있었기 때문에
파악한 정산 도메인 지식을 기반으로 모델을 기존 구현에서 도출해내었습니다. <br/>
<div style="margin-top: 10px; display: flex; justify-content: center; width: 100%">
...
1. 총 이용 금액은 운임료, 탄력요금, 호출예약비 등 `여러 항목`으로 **구성**됩니다.
2. 이용 금액은 할인에 따라 `이용자`와 `VCNC`가 **부담**합니다.
3. 총 이용 금액 중 일부는 플랫폼 수수료로 `VCNC`에게 분배되고, 나머지는 `드라이버`에게 **분배**됩니다.
<br/>
...
[comment]: <> (하지만, 정산에서는 결론적으로 `누가 누구에게` 얼마를 지불해야 하는지가 핵심입니다.)
