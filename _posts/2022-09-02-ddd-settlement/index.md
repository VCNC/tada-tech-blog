...
`정산`은 결제된 금액을 드라이버와 VCNC에 분배하는 과정입니다. 
...
하지만 결과적으로는 `누가 누구에게` 얼마를 지불해야 하는지가 핵심입니다. 기존 구현에서는 전체 흐름에 걸쳐 세 가지 기준을 모두 고려하고 있었기 때문에
도메인 지식의 복잡성이 구현에 그대로 드러나게 되었습니다.
