...
저희 팀에서도 현재 가지고 있는 문제점에 대해서 고민을 하였습니다. 평소 SwiftUI Preview를 잘 활용하지 못해 UI 작업 디버깅에 비교적 긴 시간이 걸렸던 문제가 있었고, 이를 해결할 수 있는 **UI 모듈**을 도입하기로 하였습니다.
...
기존 타다 프로젝트의 문제는 UI가 있는 모듈이 UI에 필수적인 코드를 제외한 RIBs 컴포넌트, 네트워크 등 Core한 로직 등 많은 코드들을 바라봐야 한다는 문제점이 있었습니다. 그렇기 때문에 SwiftUI Preview는 해당 모듈에 종속성이 있는 모든 코드를 컴파일 해야 하기 때문에 로딩하는 데에 굉장한 비효율이 발생하였습니다.
...
타다 iOS팀은 모듈화를 통해 SwiftUI Preview 성능을 극대화시킬 수 있었고, UI 개발의 효율성을 극대화하였습니다. Mock 데이터를 활용한 Hot Reload와 작은 단위의 UI 컴포넌트를 독립적으로 관리함으로써, 개발 생산성은 물론 유지보수성도 크게 향상되었습니다. UI 개발 뿐만 아니라 Feature 모듈을 활용한 디버깅 효율성에 대해 지속적으로 고민하고 있습니다. 
...
UI 디버깅에 있어 Example 앱을 만들어 관리하기에 부담되는 등 고민이 있으신 분들에게 도움이 됐길 바랍니다. 
