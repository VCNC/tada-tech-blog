...
모듈화는 현재 iOS 개발 생태계에서 가장 핫한 키워드입니다. 많은 회사의 iOS 팀에서 모듈화를 도입하고 있으며, 이를 통해 빌드 속도 향상, 의존성 분리, 협업 효율화 등을 기대하게 됩니다. 각 팀의 특성과 프로젝트 상황에 따라 그 효과가 크게 달라질 수 있습니다.

많은 개발자들이 모듈화의 필요성은 느끼지만, 실제로 자신들의 프로젝트에 어떠한 이점이 있을지 확신하기 어렵습니다. 대부분의 모듈화 관련 글은 추상적인 이점이나 모듈화의 도입 방법에 집중되어 있으며, 실제 적용 사례나 구체적인 장점에 대한 설명이 부족한 경우가 많습니다. Feature 단위의 모듈로 나누어진 슈퍼 앱의 사례를 보더라도, 개발 리소스나 프로젝트 상황을 고려할 때 이러한 접근이 현실적으로 와닿지 않을 수 있습니다.

타다가 왜 모듈화를 도입했는지, 그리고 이를 어떻게 효과적으로 적용하고 있는지에 대한 구체적인 경험을 2부에 나누어 포스팅할 예정입니다.

※ 타다는 Tuist를 이용하여 모듈화를 적용하였습니다.

- **1부: iOS Preview 모듈화 및 활용**
...
### Preview 모듈의 도입하게 된 배경

개발자들은 프로젝트 규모가 커질수록 디버깅의 효율화에 대해 많은 고민을 합니다. 이는 빌드 속도 증가부터 복잡한 상태 관리에 이르기까지 다양한 문제로 인해 불편함을 겪습니다. 따라서 개발자들은 여러 가지 방법을 통해 디버깅 효율성을 개선하려고 노력합니다.

예를 들면 프로젝트 규모 큰 슈퍼 앱에서는 Microfeature Architecture를 활용하여 Feature 단위의 모듈화를 통해 디버깅 및 테스팅를 최적화합니다. 이러한 구조는 복잡한 앱을 개발하는 개발자들에게 큰 이점을 제공하며, 합리적인 방식이라고 생각합니다.

저희 팀에서는 프로젝트 규모 및 개발 리소스 등을 고려하여 투자 대비 높은 효율을 낼 수 있는 방식을 고민했습니다. 평소 SwiftUI Preview를 잘 활용하지 못해 UI 작업 디버깅에 긴 시간이 걸렸던 문제가 있었고, 이를 해결할 수 있는 **Preview 모듈**을 도입하기로 하였습니다.

### 기존 SwiftUI Preview의 문제점
...
프로젝트 빌드 시에 프로젝트에서 사용하는 Third-Party 라이브러리를 포함합니다. 이는 Preview 로딩 속도를 지연시킵니다. 특히, 일부 Third-Party 라이브러리가 Xcode의 SwiftUI Preview와 완전히 호환되지 않는 경우, 정상적으로 해당 기능을 사용하지 못합니다.
...
하지만 **실제 서비스 환경**에서는 단순한 SwiftUI 컴포넌트만으로 모든 UI를 구현하는 데 한계가 있습니다. 데이터 모델, 커스텀된 UI 컴포넌트, 유틸리티 코드 등 여러 코드 파일에 의존성이 생깁니다. 

기존 프로젝트의 문제는 App 또는 App Extension Target에서 UI 코드를 가지고 있었고, 그렇기 때문에 프로젝트 내의 모든 코드를 바라봐야 한다는 문제점이 있었습니다. 그렇기 때문에 SwiftUI Preview를 로딩하는 데에 비효율이 발생하게 됩니다.

따라서 저희는 Preview 모듈을 만들었고, UI에 실질적으로 필요한 모듈을 분리함으로써 불필요한 모듈의 종속성을 제거하였습니다.

그 결과 타다의 모듈화된 구조는 아래와 같이 구성되었습니다.
...
<figcaption>타다 모듈화 구조</figcaption>
...
- **TadaRiderUI(Preview 모듈)**: App 또는 App Extension 타겟에서 실직적으로 사용되는 UI 요소와 UI 컴포넌트들로 구성되어 있습니다. 이 모듈에서 SwiftUI Preview를 사용하게 됩니다.
...
위와 같이 모듈을 분리함으로써, SwiftUI Preview를 사용하는 Preview 전용 모듈은 실제 UI를 그리는 데 필요한 Shared및 CoreUI 모듈을 바라보게 됩니다. 그 결과 SwiftUI Preview 성능이 극대화 되었고 개발 퍼포먼스가 크게 향상되었습니다.
...
위 예시에서 볼 수 있듯이, Mock 데이터를 활용하여 다양한 상태를 손쉽게 설정하고 테스트할 수 있습니다. 이 방식은 수많은 상태에 따른 UI를 확인할 때 매우 유용하며 개발 시간을 단축할 수 있습니다.
...
타다 iOS팀은 Preview 모듈을 통해 SwiftUI Preview 성능을 극대화시킬 수 있었고, UI 개발의 효율성을 극대화하였습니다. Mock 데이터를 활용한 Hot Reload와 작은 단위의 UI 컴포넌트를 독립적으로 관리함으로써, 개발 생산성은 물론 유지보수성도 크게 향상되었습니다. 이와 같은 전략은 대규모 프로젝트에서 특히 유용하며, 다양한 상태를 효과적으로 관리하고 빠르게 변화에 대응할 수 있는 기반이 됩니다.
