...
많은 iOS 개발자들이 다양한 이점을 기대하며 모듈화를 도입하고 있습니다. 저희 팀도 이러한 이점들을 기대하며 모듈화를 적용했으며, 특히 저희 팀의 개발 리소스와 프로젝트 특성에 맞게 어떻게 더 효과적으로 활용할 수 있을지 고민했습니다. 그 결과로 얻은 구체적인 경험을 2부에 걸쳐 공유하고자 합니다.
...
타다 프로젝트에서는 SwiftUI를 적극적으로 도입하고 있지만, 이러한 Preview 기능이 원활하게 동작하지 않는 문제가 있었습니다. 프로젝트의 규모가 크다 보니 단순 Preview 로딩에도 상당한 시간이 소요되었습니다. 또한, Third-Party 라이브러리와 매크로 사용으로 인해, 의도치 않게 Preview가 충돌하거나 무한 로딩 상태에 빠지는 등의 문제가 발생해 정상적인 활용이 어려웠습니다.

SwiftUI Preview를 사용하지 못했기 때문에, UI 작업을 디버깅하는 데 많은 시간이 걸렸습니다. 특히, 드라이버와의 매칭 및 실시간 통신 등 Server-Driven 상태를 처리하는 과정에서 단순한 UI 디버깅에 예상보다 더 많은 시간이 소요되는 경우가 빈번했습니다. 이러한 문제를 해결하기 위해 UI 모듈화를 도입하여 디버깅 효율성을 높이기로 했습니다.
...
타다 iOS 팀은 모듈화를 통해 SwiftUI Preview의 성능을 극대화하고, UI 개발의 효율성을 크게 향상시킬 수 있었습니다. 특히 Mock 데이터를 활용한 Hot Reload와 작은 단위의 UI 컴포넌트를 독립적으로 관리함으로써, 개발 생산성뿐만 아니라 유지보수성도 크게 개선되었습니다. 이처럼 SwiftUI Preview를 통해 빠르고 반복적인 UI 테스트가 가능해져 타다에서 진행하는 다양한 UX/UI 실험에서도 유용하게 활용되고 있습니다. 현재는 UI 개발 뿐만 아니라 Feature 모듈을 활용한 디버깅 효율성에 대해 지속적으로 고민하고 있습니다. 

타다 iOS 팀은 공통 모듈을 분리해 Feature 단위로 UI 모듈을 구현했지만, 이런 작업에는 많은 리소스가 소요될 수 있고 개발 리소스와 프로젝트 상황을 고려했을 때 적합하지 않을 수 있습니다. 이런 경우, UI와 필요한 모듈만 간단히 분리하여 SwiftUI Preview 기능을 먼저 활용하는 것도 좋은 선택일 수 있습니다. 이후, 상황에 맞게 점진적으로 모듈화를 진행하는 것을 추천드립니다.
...
SwiftUI를 활용하여 UI 작업을 하면서 디버깅에 어려움을 겪고 계신 분들에게 도움이 되길 바랍니다.
